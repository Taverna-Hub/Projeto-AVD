...
def main():
    ...
    parser.add_argument(
        "--skip-metadata",
        action="store_true",
        help="Não incluir metadados de gráficos"
    )
    parser.add_argument(
        "--anos",
        nargs="+",
    ...
    for device in resultado_devices['devices']:
        logger.info(f"   - {device['nome']}: {device['device_id'][:8]}... (token: {device['token'][:8]}...)")
    ...
    if not args.skip_telemetry:
        ...
        if args.estacoes:
            devices_to_process = [
                d for d in devices_to_process
                if d['nome'] in args.estacoes
            ]
        ...
        for idx, device in enumerate(devices_to_process, 1):
            ...
            if args.max_registros:
                logger.info(f"   Limitando a {args.max_registros} registros por arquivo")
            ...
            if resultado['registros_enviados'] > 0:
                taxa_sucesso = (resultado['sucesso'] / resultado['registros_enviados'] * 100)
                logger.info(
                    f"   ✅ {device['nome']}: "
                ...
                logger.warning(f"   ⚠️ {device['nome']}: Nenhum arquivo encontrado ou processado")
