...
class ThingsBoardClient:
    ...
    def get_device_by_name(self, name: str) -> Optional[Dict]:
        ...
        """
        url = (
            f"{self.base_url}/api/tenant/devices?pageSize=100&page=0&textSearch={name}"
        )
    ...
    def create_device(
        self,
        name: str,
        device_type: str = "weather_station_processed",
        label: str = None,
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Erro ao criar device '{name}': {e.response.status_code} - {error_detail}"
                )
            except:
                logger.error(
                    f"Erro ao criar device '{name}': {e.response.status_code} - {e.response.text}"
                )
            return None
        except Exception as e:
